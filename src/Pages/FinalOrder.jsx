    ...
    const fetchFinalOrder = async () => {
      try {
        ...
        const newArr = res.data.filter(
          (el) => el.dateMade.slice(0, 11) == currDate
        );
      ...
      {finalOrder.length > 0 &&
        finalOrder.map((dish) => {
          if (Date().toString().slice(0, 11) == dish.dateMade.slice(0, 11))
            return (
              <div key={dish._id + Math.random() * 1000} className="dish">
                <h4>{dish.title}</h4>
                <p>
                  <span className="span-data"> Servings</span> : {dish.quantity}
                </p>
                <p>
                  {" "}
                  <span className="span-data"> Date Ordered : </span>{" "}
                  {dish.dateMade.slice(0, 11)}
                </p>
                {Date().toString().slice(0, 11) !==
                  dish.dateMade.slice(0, 11) && <div>Old Order</div>}
                {Date().toString().slice(0, 11) ==
                  dish.dateMade.slice(0, 11) && (
                  <div>
                    <span className="span-data"> Status : </span>Active order 🍽️
                  </div>
                )}
                ...
                <div className="finalOrderBtns">
                  {Date().toString().slice(0, 11) ==
                    dish.dateMade.slice(0, 11) && (
                    <button onClick={() => UpdateOrder(dish._id)}>
                      Update Order
                    </button>
                  )}
                  {Date().toString().slice(0, 11) ==
                    dish.dateMade.slice(0, 11) && (
                    <button onClick={() => DeleteOrder(dish._id)}>
                      Delete Order
                    </button>
                  )}
                </div>
